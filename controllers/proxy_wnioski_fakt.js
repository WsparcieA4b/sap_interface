...
const repo = require("../service/proxy_wnioski_fakt");
const shared = require('../service/a_common_db_lib')

router.post('/proxyzf', async function (req, res, next) {
    const isNew = fprx.isNewSalesOrder(req.body.body)
    const url = fprx.prepUrl(isNew)
    logger.info('[proxy_zf.post] url: ' + url)
    logger.info('[proxy_zf.post] req.body: ' + JSON.stringify(req.body))
    try {
        const response = await fprx.fetchToken(url, conn.passphrase, conn.pfxFilePath)
        if (response.ok) {
            console.log('[S4_FETCH_TOKEN] [proxy_zf.post]  Response OK from S4!')
            const csrfToken = response.headers.get('x-csrf-token')
            const cookieInp = response.headers.get("set-cookie")
            const cookie = fprx.prepCookie(cookieInp)
            await postData(res, req.body, url, cookie, csrfToken, isNew, conn.passphrase, conn.pfxFilePath)
        } else {
            const obj = {
        ...
        logger.info('[proxy_zf.post] error ', err)
        console.log(err)
        return next(err)
    }
});
...
async function postData(res, data, url, cookie, token, isNew, passphr, pfxFilePath) {
    ...
    logger.info('[proxy_zf.post] id:' + data.id)
    logger.info('[proxy_zf.post] ' + JSON.stringify(so_data))
    ...
    console.log("Dane ZF do SAP:", requestOptions.body)
    ...
    try {
        ...
        logger.info(result)
        console.log(result)
        const xmler = result.startsWith('<?xml') || result.startsWith('Caused by:java')
        if (!isOK) {
            //bład po stronie SAP
            await shared.SAP_Logger_Save_WF_Call2IntegrationLog(data.id, `Nieudany zapis do SAP. Status: ${rcode}`, statusText + ' ' + result, xmler ? 'Warning' : 'Info', null)
            res.status(rcode || 200).json({ status: rcode || 200, error: false, message: `Update done. Status: ${rcode}` });
        }
        else if (!isNew) {
            console.log('Update done!')
            await shared.SAP_Logger_Save_WF_Call2IntegrationLog(data.id, `Zlecenie SAP zaktualizowane. Status: ${rcode}`, statusText + ' ' + result, xmler ? 'Warning' : 'Info', null)
            res.status(rcode || 200).json({ status: rcode || 200, error: false, message: `Update done. Status: ${rcode}` });
        }
            ...
            try {
                ...
                //logger dla UI
                await shared.SAP_Logger_Save_WF_Call2IntegrationLog(data.id, "Nowe zlecenie SAP zapisane " + so, so, 'Info', null)
                ...
                await shared.SAP_Logger_Save_WF_Call2IntegrationLog(data.id, "Zapis nr zlecenia erp nieudany ", '-', 'Error', null)
                res.status(500).json({ status: 500, error: true, message: jsnError });
            }
        }
        else {
            //logger błedu SAP
            await shared.SAP_Logger_Save_WF_Call2IntegrationLog(data.id, "Błąd zapisu zlecenia do SAP!", result, 'Error', null)
            // throw Error(result);
        }
        ...
        await shared.SAP_Logger_Save_WF_Call2IntegrationLog(data.id, "Inny błąd zapisu do SAP!", error ? error.message : 'Empty error property!', 'Error', null)
