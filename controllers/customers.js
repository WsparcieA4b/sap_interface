...
router.post('/customer', function (req, res) {
  logger.info('[customers.post] /customer ' + JSON.stringify(req.body))
  let ret_object = { status: 200, error: false }
  let dataArr = utils.ensureArray(req.body.Record)

  logger.info('[customers.post] input array size: ' + dataArr.length)

  //Bluebird promise lib
  BPromise.map(dataArr, (cust) => {
    return service.insertOrUpdate({ ...cust, 'vendor': null, 'customer': true })
  }, { concurrency: 3 }
  ).then((done) => {
