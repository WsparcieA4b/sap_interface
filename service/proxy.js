...
module.exports = {
    insertOrUpdate: async function (sapOrderNo, serviceId) {
        const tableName = 'se_service_calls'
        ...
        try {
            ...
            if (!!so_exists) {
                var res = await db(tableName).where(sel).update(dbdata, "sap_order_no")
                return { result: res, error: false, message: `Identyfikator SAP ${sapOrderNo} zapisany do se_service_calls, id: ${serviceId}` }
            } else {
    ...
    isNew: async function (serviceId) {
        ...
        if (!!so_exists) {
            ...
            if (sap_order_no > 0) return false
        }
        return true
    }
}
